// ============================================================
//  Loyalty program rules
//  Earn:   1 point per 1 unit spent (floor)
//  Redeem: blocks of 100 points are worth 10 units
//  Tiers:  BRONZE < 1000 | SILVER >= 1000 | GOLD >= 5000 (lifetime earned)
//  Tier perks: SILVER earns 1.1x points, GOLD earns 1.3x points
// ============================================================

export const POINTS_PER_UNIT = 1;
export const REDEMPTION_POINTS = 100;
export const REDEMPTION_VALUE = 10;

export const TIER_THRESHOLDS = { BRONZE: 0, SILVER: 1000, GOLD: 5000 } as const;

export const TIER_MULTIPLIER: Record<LoyaltyTier, number> = {
  BRONZE: 1,
  SILVER: 1.1,
  GOLD: 1.3,
};

export const TIER_DISCOUNT: Record<LoyaltyTier, number> = {
  BRONZE: 0,
  SILVER: 2, // percent off
  GOLD: 5, // percent off
};

export type LoyaltyTier = 'BRONZE' | 'SILVER' | 'GOLD';

export function tierFor(totalEarned: number): LoyaltyTier {
  if (totalEarned >= TIER_THRESHOLDS.GOLD) return 'GOLD';
  if (totalEarned >= TIER_THRESHOLDS.SILVER) return 'SILVER';
  return 'BRONZE';
}

// Points earned on a sale of `amount` given the customer's tier
export function pointsForAmount(amount: number, tier: LoyaltyTier = 'BRONZE'): number {
  return Math.floor((Math.max(amount, 0) / POINTS_PER_UNIT) * TIER_MULTIPLIER[tier]);
}

// Cash value of the redeemable blocks in `points`
export function redemptionValue(points: number): number {
  return Math.floor(points / REDEMPTION_POINTS) * REDEMPTION_VALUE;
}

// The largest whole-block points amount whose value does not exceed `amount`
export function maxRedeemablePoints(amount: number): number {
  return Math.floor(Math.max(amount, 0) / REDEMPTION_VALUE) * REDEMPTION_POINTS;
}

export function tierBenefits(tier: LoyaltyTier) {
  return {
    tier,
    threshold: TIER_THRESHOLDS[tier],
    multiplier: TIER_MULTIPLIER[tier],
    discountPercent: TIER_DISCOUNT[tier],
  };
}
